...
func (s *Email) UsageInfo() string {
	...
	data := [][]string{
		...
		[]string{"username=<username>", "The smtp server authentication information - required"},
		[]string{"password=<password>", "The smtp server authentication information - required"},
		[]string{"", ""},
...
func (s *Email) Export(data []byte) error {
	...
	if port == "465" {
		...
		err = e.SendWithTLS(urlInfo.Host, smtp.PlainAuth("", username, password, urlInfo.Host), tlsConfig)

	} else {
		err = e.Send(urlInfo.Host, smtp.PlainAuth("", username, password, urlInfo.Host))
	}

	if err != nil {
		log.Errorf("Failed to send file over email - %v", urlInfo.Host)
		return err
	}
